<x-admin-app-layout>
    <div class="m-6 mx-auto shadow-lg p-5 container">
        <h1 class="text-center font-bold text-3xl text-green-600 ">Create New Category For Restaurant </h1>
        <form method="POST" action="{{url("/admin/restaurant-categories")}}" enctype="multipart/form-data">
            @csrf
            <div class="bg-gray-200 my-5 rounded-lg border mx-auto w-2/5">
                <div class="p-5">
                    <label class="font-semibold " for="name">Category Name:</label>
                    <input class="border border-gray-200 rounded-lg hover:bg-green-100"
                           value="{{old('name')}}"
                           type="text" name="name" id="name" placeholder="write here...">
                    @error('name') <p class="text-md font-bold text-red-600 ">{{$message}}</p> @enderror
                </div>
